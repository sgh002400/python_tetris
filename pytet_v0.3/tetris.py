...
class Tetris():
    ...
    def deleteFullLines(self): # To be implemented!!
        ...
        search = self.currBlk.get_dy()
        deleted = 0

        if self.top + self.currBlk.get_dy() >= self.iScreenDy :
            search = self.iScreenDy - self.top
        

        arrayScreen = self.createArrayScreen()
        temp_Screen = Matrix(arrayScreen)
        zero = temp_Screen.clip(0, 0, 1, temp_Screen.get_dx())

        for i in range(search-1,-1,-1):
